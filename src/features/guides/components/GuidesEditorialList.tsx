...
// SkillBar: анимация ширины прогресс-бара через CSS transition + useInView.
// Заменяет: <motion.div initial={{ width: 0 }} whileInView={{ width: `${value}%` }} />
const SkillBar = ({ label, value, icon: Icon, colorClass }: {
  label: string;
  value: number;
    ...
    <div className="mb-2">
      ...
      <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
        <div
          ...
          className={cn(
            'h-full rounded-full shadow-[0_0_10px_currentColor] transition-[width] duration-1000 ease-out',
            bgClass
          )}
        />
      </div>
...
export default function GuidesEditorialList({ guides = [] }: { guides: Guide[] }) {
  ...
  const { ref: refCta, inView: ctaInView } = useInView();
    ...
    <div className="container mx-auto px-4 max-w-7xl">
      <div className="flex flex-col gap-24 md:gap-32 pb-24">
        ...
        <div
          ref={refCta}
          className={cn(
            'relative mt-12 bg-slate-900/60 backdrop-blur-xl border border-white/5 rounded-[3rem] p-8 md:p-16 text-center overflow-hidden shadow-2xl transition-all duration-700 ease-out',
...
function EditorialGuideBlock({ guide, index }: { guide: Guide, index: number }) {
  ...
  const { ref, inView } = useInView({ rootMargin: '-100px', threshold: 0.1 });
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-center relative transition-all duration-700 ease-out',
