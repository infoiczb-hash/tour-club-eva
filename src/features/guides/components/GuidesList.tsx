...
import { Instagram, Send, X, ArrowRight, ShieldCheck, MapPin, User, ChevronRight, Zap, Flame, Sparkles, Utensils, Activity, Heart, Compass } from "lucide-react";
...
const ICON_MAP: Record<string, { icon: React.ElementType, color: string }> = {
  Zap: { icon: Zap, color: "text-amber-400" }, Utensils: { icon: Utensils, color: "text-rose-400" },
  Sparkles: { icon: Sparkles, color: "text-purple-400" }, Flame: { icon: Flame, color: "text-teal-400" },
  Activity: { icon: Activity, color: "text-sky-400" }, Heart: { icon: Heart, color: "text-red-500" },
  Compass: { icon: Compass, color: "text-emerald-400" },
};

interface Guide {
  id: string; slug: string; name: string; role: string;
  image: string | null; actionImage: string | null; bio: string | null; fullBio: string | null;
  superpower: string | null; experience: string | null;
  tags: string[]; achievements: string[]; quotes: string[]; stats: any;
  instagram: string | null; telegram: string | null; contact: string | null;
  order: number; isActive: boolean;
}

// SkillBar — AnimatePresence/motion.div оставляем: это анимация при открытии модалки (UI)
  ...
  <div className="mb-2">
    <div className="flex justify-between items-end mb-2">
      <div className="flex items-center gap-2 text-[12px] font-bold uppercase tracking-widest text-slate-400">
        <Icon size={14} className={colorClass} /><span>{label}</span>
      </div>
  ...
  return (
    <section className="py-12 md:py-20 bg-slate-950 text-white relative overflow-hidden" id="team">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        {/* HEADER — whileInView заменён на useInView */}
          ...
          <div className="max-w-2xl">
            ...
            >
              ...
              <p className="text-slate-400 text-s md:text-base font-medium max-w-md leading-relaxed border-l-2 border-white/10 pl-4">
                Профессионалы, с которыми безопасно и интересно в любой точке мира. Знают каждый камень на маршруте.
              </p>
            </div>
          </div>
        </div>
        ...
        {/* GRID */}
        <div
          tabIndex={0} role="region" aria-label="Список команды гидов"
          className="flex overflow-x-auto snap-x snap-mandatory hide-scrollbar gap-4 -mx-4 px-4 pb-8 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal-500/50 rounded-2xl md:grid md:grid-cols-2 lg:grid-cols-4 md:gap-6 md:mx-0 md:px-0 md:pb-0"
        >
          {displayGuides.map((guide, idx) => (
            <GuideCard key={guide.id} guide={guide} index={idx} onClick={() => setSelectedGuide(guide)} />
          ))}

          {/* JOIN CARD — whileInView заменён на useInView */}
          <div
            ref={joinView.ref}
            style={{ opacity: joinView.inView ? 1 : 0, transform: joinView.inView ? 'translateY(0)' : 'translateY(20px)', transition: 'opacity 0.6s ease 0.2s, transform 0.6s ease 0.2s' }}
            onClick={() => openContactModal(undefined, 'HR')}
            className="relative group flex-shrink-0 cursor-pointer snap-center w-[85vw] sm:w-[300px] aspect-[3/4] md:w-auto md:aspect-auto md:h-[450px] rounded-[2rem] border-2 border-dashed border-white/10 hover:border-teal-500/50 bg-white/[0.02] hover:bg-teal-900/10 transition-all duration-300 flex flex-col items-center justify-center text-center p-6 md:mt-0 lg:mt-8"
          >
      ...
      {/* HERO MODAL — AnimatePresence + layoutId оставляем (UI анимация) */}
      <AnimatePresence>
        {selectedGuide && <GuideHeroModal guide={selectedGuide} onClose={() => setSelectedGuide(null)} />}
      </AnimatePresence>
    </section>
  );
}
...
function GuideCard({ guide, index, onClick }: { guide: Guide, index: number, onClick: () => void }) {
  ...
  return (
    // layoutId оставляем — нужен для shared element transition с модалкой
    // whileInView убран → карточки в горизонтальном скролле, IntersectionObserver на каждую = плохо
    <motion.div
    ...
    >
      {guide.image ? (
        <Image src={guide.image} alt={guide.name} fill className="object-cover transition-all duration-700 md:grayscale md:group-hover:grayscale-0 md:scale-100 md:group-hover:scale-110" sizes="(max-width: 768px) 100vw, 25vw" />
      ) : (
        <div className="w-full h-full bg-slate-800 flex items-center justify-center"><User size={64} className="text-slate-700" /></div>
      )}
      ...
      <div className="absolute inset-0 p-6 md:p-8 flex flex-col justify-end">
        <div className="transform translate-y-2 md:translate-y-8 group-hover:translate-y-0 transition-transform duration-500">
          <span className="inline-block px-2 py-0.5 rounded bg-teal-500 text-slate-900 text-[12px] font-black uppercase tracking-widest mb-3 shadow-lg">{guide.role}</span>
          <h3 className="text-2xl md:text-3xl font-black text-white leading-none uppercase tracking-tight mb-2">{guide.name}</h3>
          {guide.bio && (
            <p className="text-xs text-slate-300 font-medium line-clamp-2 mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 hidden md:block">{guide.bio}</p>
          )}
          <div className="flex items-center gap-2 text-teal-400 text-[12px] font-bold uppercase tracking-widest opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity duration-300 mt-4">
            <span>Смотреть профиль</span><ChevronRight size={14} />
          </div>
        </div>
      </div>
    </motion.div>
...
function GuideHeroModal({ guide, onClose }: { guide: Guide, onClose: () => void }) {
  ...
  if (guide.stats) {
    ...
    catch (e) { console.error("Ошибка парсинга статов", e); }
  }
      ...
      >
        ...
        <div className="w-full md:w-5/12 h-[40vh] md:h-auto relative">
          {guide.actionImage || guide.image ? (
            <Image src={guide.actionImage || guide.image || ''} alt={guide.name} fill className="object-cover" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" quality={85} />
          ) : (
        ...
        <div className="w-full md:w-7/12 p-6 md:p-10 flex flex-col h-[60vh] md:h-auto bg-slate-950 relative">
          ...
          <div className="flex-1 overflow-y-auto pr-4 -mr-2 scrollbar-thin scrollbar-thumb-slate-800 pb-24 md:pb-32">
            <div className="flex flex-wrap gap-2 mb-8">
              ...
              {guide.tags && guide.tags.map((tag, idx) => (
                <span key={idx} className="px-3 py-1 bg-slate-900 border border-slate-800 text-slate-400 text-[10px] font-bold uppercase tracking-wider rounded">{tag}</span>
              ))}
            </div>
            ...
            {parsedStats.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6 mb-10 p-6 bg-white/[0.03] rounded-3xl border border-white/5 shadow-inner">
                {parsedStats.map((stat, i) => {
                  ...
                  return <SkillBar key={i} label={stat.label} value={stat.value} icon={mapping.icon} colorClass={mapping.color} />;
                })}
              </div>
            )}
            ...
            {guide.quotes && guide.quotes.length > 0 && (
              <div className="mb-8 border-l-2 border-teal-500 pl-4 py-1">
                <p className="text-lg md:text-xl font-medium text-white italic leading-snug">«{guide.quotes[0]}»</p>
              </div>
            )}
          ...
          <div className="absolute bottom-0 left-0 right-0 p-6 md:px-10 md:pb-10 bg-gradient-to-t from-slate-950 via-slate-950 to-transparent">
            <Link href="/tour" className="w-full py-4 rounded-xl bg-teal-500 hover:bg-teal-400 text-slate-950 font-black uppercase tracking-wider flex items-center justify-center gap-2 transition-all shadow-[0_0_20px_rgba(20,184,166,0.3)] hover:shadow-[0_0_30px_rgba(20,184,166,0.5)] active:scale-[0.98]">
              <MapPin size={18} strokeWidth={2.5} /><span>ТУРЫ СО МНОЙ</span>
            </Link>
          </div>
        </div>
      </motion.div>
