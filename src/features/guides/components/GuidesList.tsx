...
// SkillBar в модале: animate={{ width }} оправдан — это реакция на mount модала,
// а не scroll-анимация. Framer остаётся только здесь.
...
export default function GuidesList({ guides = [] }: { guides: Guide[] }) {
  ...
  const { ref: refJoin, inView: joinInView } = useInView();
  ...
  return (
    <section className="py-12 md:py-20 bg-slate-950 text-white relative overflow-hidden" id="team">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          ...
          <div
            ...
            role="button"
            tabIndex={0}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') openContactModal(undefined, 'HR'); }}
            aria-label="Подать заявку в команду"
            style={{ transitionDelay: joinInView ? '200ms' : '0ms' }}
            className={cn(
              ...
              'bg-white/[0.02] hover:bg-teal-900/10 transition-all duration-500 ease-out',
              'flex flex-col items-center justify-center text-center p-6',
              'md:mt-0 lg:mt-8',
              joinInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'
            )}
          >
      ...
      {/* HERO MODAL — AnimatePresence + layoutId оправданы: exit-анимация и shared layout */}
      <AnimatePresence>
        {selectedGuide && (
...
// GuideCard: layoutId нужен для shared layout анимации с модалом — Framer остаётся.
// whileInView убран — заменён на useInView + CSS.
function GuideCard({ guide, index, onClick }: { guide: Guide, index: number, onClick: () => void }) {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group relative flex-shrink-0 cursor-pointer snap-center transition-all duration-500 ease-out",
        "w-[85vw] sm:w-[300px] aspect-[3/4] md:w-auto md:aspect-auto md:h-[450px]",
