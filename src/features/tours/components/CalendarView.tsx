...
import { 
  MapPin, Clock, ChevronRight, Calendar as CalendarIcon, Sparkles
} from 'lucide-react';
...
// Строгий тип для "Развернутого" тура
    ...
    <div className="w-full mx-auto pb-20">
      ...
      {monthKeys.map((month) => (
        <div key={month} className="mb-10 relative">
            ...
            {/* Секционный разделитель месяца (Premium UI) */}
            ...
            <div className="flex flex-col gap-3 sm:gap-4">
                {groupedTours[month].map((tour) => (
                    <CalendarRow key={tour.uniqueId} tour={tour} />
                ))}
            </div>
        </div>
      ))}
...
// =======================================================
// УЛУЧШЕННЫЙ COMPONENT СТРОКИ (PREMIUM SLAB CARD)
// =======================================================
        ...
        <Link href={`/tour/${tour.slug}`} className="group block outline-none">
            <motion.div 
                ...
                className={cn(
                    ...
                    "bg-[#0d131a] border-2 border-white/5 hover:border-teal-500/30 hover:bg-slate-900/80 hover:shadow-2xl",
                    isTba && "opacity-70 grayscale hover:grayscale-0 hover:opacity-100"
                )}
            >
                {/* 1. АРХИТЕКТУРНЫЙ БЛОК ДАТЫ (Слева) */}
                ...
                {/* 2. МИНИАТЮРА ФОТО (Только Десктоп - мощный визуальный триггер) */}
                ...
                <div className="flex flex-col justify-center flex-1 min-w-0 py-1">
                    ...
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[12px] sm:text-xs font-bold uppercase tracking-wider text-slate-400 mb-1.5">
                        ...
                        {isMultiDay && endDateObj && (
                            <span className="flex items-center text-slate-500 before:content-['•'] before:mx-1.5 before:text-slate-700">
                    ...
                    {/* Крупный Заголовок */}
                    <h4 className="text-base sm:text-xl font-black text-white leading-tight line-clamp-2 pr-2 group-hover:text-teal-300 transition-colors mb-2">
                        {tour.title}
                    </h4>
                ...
                <div className="flex items-center justify-end gap-3 sm:gap-6 shrink-0 pl-1 sm:pl-4">
                    ...
                    {/* Явная кнопка (Chevron) - теперь видна всегда, даже на мобильных! */}
