...
export default function TourProgram({ program }: TourProgramProps) {
  ...
  const [isPrinting, setIsPrinting] = useState(false); // 🔥 Специальный стейт для печати

  const toggleDay = (index: number) => setOpenDayIndex(openDayIndex === index ? null : index);
  
  // 🔥 Правильный слушатель печати
  useEffect(() => {
    const handleBeforePrint = () => setIsPrinting(true);   // Перед печатью: открыть всё
    const handleAfterPrint = () => setIsPrinting(false);   // После печати: вернуть как было
      ...
      <div className="relative max-w-4xl">
        ...
        <div className="space-y-3">
          {days.map((day: any, index: number) => {
            // 🔥 Дни открываются, если мы печатаем ИЛИ если пользователь кликнул на день
            const isOpen = isPrinting || openDayIndex === index;
            const isLast = index === days.length - 1;

            return (
              // 🔥 Добавлен print:break-inside-avoid (защита от разрыва дня на бумаге)
              <div key={index} className="relative pl-14 md:pl-20 print:pl-12 print:break-inside-avoid print:mb-4">
                ...
                >
                  ...
                  >
                    ...
                    {/* Скрываем стрелочку при печати */}
                    <ChevronDown 
                        size={18} 
                        className={clsx("text-slate-400 transition-transform duration-300 print:hidden", isOpen && "rotate-180 text-teal-500")} 
                    />
                  </div>
                  ...
                  <AnimatePresence>
                    {isOpen && (
                      <motion.div
                        ...
                        // 🔥 Если идет печать, открываем мгновенно (duration: 0)
        ...
        <div className="mt-6 pl-14 md:pl-0 print:break-inside-avoid">
          <div className="bg-amber-900/10 border border-amber-500/20 rounded-2xl p-4 md:p-5 flex flex-col md:flex-row gap-3 md:gap-4 items-start print:bg-white print:border-slate-300">
            ...
            <div>
               <h4 className="text-amber-500 font-bold uppercase text-xs md:text-sm mb-1 tracking-wide print:text-slate-900">
                 Внимание!
               </h4>
