...
export default function TourWishlistButton({ tourId, initialIsWished, className }: Props) {
  ...
  const handleToggle = async (e: React.MouseEvent) => {
    ...
    // Оптимистичное обновление UI (сразу красим сердечко)
    ...
    startTransition(async () => {
      ...
      if (res.needsAuth) {
        setIsWished(initialIsWished); // Откатываем UI назад
      ...
      router.refresh(); // Тихо обновляем серверные данные страницы
    });
  };
