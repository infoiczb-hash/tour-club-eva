...
export default function TourDates({ tour }: TourDatesProps) {
  ...
  // Берем массив дат из новой реляционной модели
  const datesToRender = tour.tourDates || [];
    ...
    <section className="scroll-mt-24 mb-12 md:mb-16" id="dates">
      ...
      <div className="flex flex-col gap-3">
        {datesToRender.map((item) => {
           ...
           // Формируем красивую строку для формы бронирования (например: "15 июля — 17 июля в 09:00")
           const fullDateStringForBooking = `${dateString}${item.time ? ` в ${item.time}` : ''}`;

           const isSoldOut = item.spotsLeft <= 0;
           
           // Берем гида (пока берем дефолтного из тура)
           ...
           return (
            <div 
               key={item.id} 
               // ✅ ИСПРАВЛЕНО: Вызываем Zustand и передаем ID конкретной даты (item.id)
            ...
            >
                ...
                {/* ПРАВАЯ ЧАСТЬ: ГИД, СТАТУС И СТРЕЛКА */}
                    ...
                    <div className="flex items-center gap-4">
                        {isSoldOut ? (
                            ...
                            <div className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-widest bg-slate-950/50 border border-white/5 px-2.5 py-1.5 rounded-md">
                                ...
                                <span className="text-emerald-400">Осталось {item.spotsLeft}</span>
                            </div>
                        )}
