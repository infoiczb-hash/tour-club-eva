        ...
        {datesToRender.map((item) => {
          ...
          return (
            <div 
              ...
              aria-label={isSoldOut ? `${dateString} — мест нет` : `Записаться на ${dateString}`}
              aria-disabled={isSoldOut}
              onClick={() => !isSoldOut && openBookingModal(tour, fullDateStringForBooking, item.id)}
              onKeyDown={(e) => {
                if (!isSoldOut && (e.key === 'Enter' || e.key === ' ')) {
                  e.preventDefault();
                  openBookingModal(tour, fullDateStringForBooking, item.id);
                }
              }}
            ...
            >
              ...
              <div className="flex items-center justify-between md:w-2/3 md:justify-end md:gap-8">
                ...
                {/* ГИД — p вместо h3, это лейбл данных а не заголовок страницы */}
