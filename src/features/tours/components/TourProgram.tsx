...
import { TourProgramData, TourProgramDay, TourActivity } from "../types"; // Убедитесь, что импорты верны
...
export default function TourProgram({ program }: { program: TourProgramData }) {
  ...
  if (typeof program === 'string') {
    ...
    // Приведение типа, так как TS не уверен в содержимом 'days'
              ...
              <div className="bg-white dark:bg-slate-900 p-6 md:p-5 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-all duration-300">
                ...
                {item.activities && item.activities.length > 0 ? (
                  <div className="space-y-4">
                    {/* Явно указываем тип TourActivity для act */}
                      ...
                      <div key={aIdx} className="flex gap-4 items-start p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 hover:border-teal-200 dark:hover:border-teal-900 transition-colors">
                        ...
                        {act.time && (
                          <div className="shrink-0 pt-0.5">
                             <span className="text-xs font-mono font-bold text-slate-400 bg-white dark:bg-slate-900 px-2 py-1 rounded-md border border-slate-200 dark:border-slate-700">
                                {act.time}
                             </span>
                          </div>
                        )}
                        ...
                        <div className="space-y-1">
                           ...
                           {/* Описание активности */}
                           {act.description && (
                             <p className="text-xs md:text-sm text-slate-400 dark:text-slate-400 leading-relaxed">
                               {act.description}
                             </p>
                           )}
                        </div>
                  ...
                  // Теперь это легально, так как мы добавили description в types.ts
                  item.description && (
                    <div className="prose prose-sm dark:prose-invert text-slate-600 dark:text-slate-400 leading-relaxed whitespace-pre-line">
                      {item.description}
                    </div>
                  )
                )}
