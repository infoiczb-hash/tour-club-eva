...
import { 
  ...
  ChevronRight, CheckCircle2, Clock, AlertCircle, QrCode
} from 'lucide-react';
import { clsx } from 'clsx';
import cloudinaryLoader from '@/lib/cloudinary-loader';

interface BookingCardProps {
  booking: any; // Замени на свой тип BookingWithTour
}
    ...
    <div className="relative flex flex-col md:flex-row bg-slate-900 rounded-3xl overflow-hidden border border-white/10 shadow-xl group transition-all hover:border-white/20 hover:shadow-2xl">
      ...
      <div className="flex-1 flex flex-col sm:flex-row p-4 sm:p-6 gap-6">
        ...
        <div className="w-full sm:w-48 h-48 sm:h-auto rounded-2xl overflow-hidden relative shrink-0">
          ...
          {/* Статус на мобилке (на десктопе он в талоне) */}
      ...
      <div className="hidden md:flex flex-col items-center justify-between relative w-6 border-l-2 border-dashed border-white/10 my-4">
      ...
      <div className="md:hidden w-full h-0 border-t-2 border-dashed border-white/10 relative my-2">
      ...
      <div className="w-full md:w-64 bg-slate-800/20 p-6 flex flex-col justify-between items-center text-center relative z-10">
        ...
        <div className="p-3 bg-white rounded-xl mb-6 shadow-inner hidden md:block opacity-90 grayscale group-hover:grayscale-0 transition-all duration-500">
           <QrCode size={80} className="text-slate-900" strokeWidth={1} />
        </div>
