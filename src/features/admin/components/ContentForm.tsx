      ...
      <div className="bg-white dark:bg-slate-950 rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh] border border-slate-200 dark:border-slate-800">
        ...
        <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-white/80 dark:bg-slate-950/80 backdrop-blur rounded-t-2xl">
          <div>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-600 font-medium">Настройка контента сайта</p>
          </div>
          <button onClick={onClose} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition text-slate-600"><X size={20}/></button>
        </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                    <label className="text-xs font-bold text-slate-600 uppercase tracking-wider">Заголовок (H1)</label>
              ...
              {/* Подзаголовок */}
              <div>
                <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-1">Подзаголовок</label>
              ...
              {/* ФОН (Картинка) */}
              <div>
                <label className="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Фоновое изображение</label>
                <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-4 text-center hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors relative group overflow-hidden">
                    {data.bg_image ? (
                        ...
                        <div className="py-4 text-slate-600 flex flex-col items-center">
                ...
                {/* Fallback input */}
                <input className="w-full mt-2 p-2 text-xs bg-transparent border-b border-slate-200 dark:border-slate-800 outline-none text-slate-600 focus:border-violet-500 transition-colors"
          ...
          {slug === 'footer' && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-bold text-slate-600 uppercase mb-1 flex items-center gap-1 tracking-wider"><Mail size={12}/> Email</label>
                  ...
                  <div>
                    <label className="text-xs font-bold text-slate-600 uppercase mb-1 flex items-center gap-1 tracking-wider"><Phone size={12}/> Телефон</label>
                    <input className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm dark:text-white outline-none focus:ring-2 focus:ring-violet-500/20 transition-all"
              ...
              <div>
                <label className="text-xs font-bold text-slate-600 uppercase mb-1 flex items-center gap-1 tracking-wider"><Instagram size={12}/> Instagram (ссылка)</label>
                <input className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm dark:text-white outline-none focus:ring-2 focus:ring-violet-500/20 transition-all"
                    value={data.instagram || ''} onChange={e => handleChange('instagram', e.target.value)} placeholder="https://instagram.com/..."
                />
              </div>
               <div>
                <label className="text-xs font-bold text-slate-600 uppercase mb-1 flex items-center gap-1 tracking-wider"><Send size={12}/> Telegram (ссылка)</label>
                <input className="w-full p-3 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm dark:text-white outline-none focus:ring-2 focus:ring-violet-500/20 transition-all"
