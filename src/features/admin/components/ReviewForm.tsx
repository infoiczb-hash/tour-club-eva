    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in">
      <div className="bg-white dark:bg-slate-900 w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col no-scrollbar">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-5">
          ...
          {/* Имя */}
          <div>
            <label className="block text-[11px] font-bold text-slate-300 uppercase tracking-wider mb-1.5">Имя клиента</label>
            <input 
          ...
          <div>
            <label className="block text-[11px] font-bold text-slate-300 uppercase tracking-wider mb-1.5">Источник</label>
                 ...
                 <button
                    ...
                    className={`flex items-center justify-center gap-1.5 p-2 rounded-xl border text-[11px] font-bold transition-all ${
                      formData.source === opt.id 
                        ...
                        : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/50'
                    }`}
                 >
                    {opt.icon} {opt.label}
                 </button>
          ...
          {/* Категория тура */}
          <div>
            <label className="flex items-center gap-1.5 text-[11px] font-bold text-slate-300 uppercase tracking-wider mb-1.5">
          ...
          {/* Текст */}
          <div>
            <label className="block text-[11px] font-bold text-slate-300 uppercase tracking-wider mb-1.5">Текст отзыва</label>
            <textarea 
          ...
          >
             ...
             <div>
                ...
                <span className="text-[12px] text-slate-300 leading-tight">Если выключить, отзыв скроется из карусели</span>
             </div>
          </div>
