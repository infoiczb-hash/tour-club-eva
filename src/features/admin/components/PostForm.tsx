      ...
      <div className="bg-white dark:bg-slate-950 rounded-2xl w-full max-w-6xl shadow-2xl flex flex-col max-h-[95vh] border border-slate-200 dark:border-slate-800 relative overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-white/80 dark:bg-slate-950/80 backdrop-blur z-10">
          <div>
            ...
            <p className="text-[12px] font-bold text-slate-600 uppercase tracking-widest mt-0.5">CONTENT MANAGER</p>
          </div>
          <div className="flex gap-2">
             ...
             <button type="button" onClick={onClose} className="hover:bg-slate-100 dark:hover:bg-slate-800 p-2 rounded-xl transition text-slate-600"><X size={24}/></button>
          </div>
        </div>
        ...
        <form id="post-form" onSubmit={handleSubmit} className="p-6 sm:p-8 overflow-y-auto space-y-8 flex-1 custom-scrollbar pb-24">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="space-y-3">
                    <div>
                        <label className="block text-[12px] font-bold text-slate-600 dark:text-slate-600 uppercase mb-1.5 ml-1 tracking-wider">Title (Заголовок)</label>
                    ...
                    <div className="flex gap-2 items-end">
                        <div className="flex-1">
                             <label className="block text-[12px] font-bold text-slate-600 dark:text-slate-600 uppercase mb-1.5 ml-1 tracking-wider flex items-center gap-1">
                                <LinkIcon size={10}/> URL Slug
                             </label>
                             <input className="w-full p-2 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-lg text-xs font-mono text-slate-600 dark:text-slate-600 focus:ring-1 focus:ring-violet-500/20 outline-none" 
                        ...
                        <button 
                            ...
                            className="p-2 h-[34px] bg-slate-100 dark:bg-slate-800 rounded-lg text-slate-600 hover:text-violet-500 hover:bg-violet-50 dark:hover:bg-violet-900/20 transition-colors"
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-[12px] font-bold text-slate-600 uppercase mb-1.5 ml-1 tracking-wider flex items-center gap-1"><Tag size={10}/> Category</label>
                    ...
                    <div>
                        <label className="block text-[12px] font-bold text-slate-600 uppercase mb-1.5 ml-1 tracking-wider flex items-center gap-1"><Clock size={10}/> Read Time (min)</label>
                        <div className="relative">
                            ...
                            />
                            <div className="absolute right-3 top-3 text-[12px] text-slate-600 font-bold uppercase pointer-events-none">Auto-calc</div>
                        </div>
                    </div>
                </div>

                <div>
                    <label className="block text-[12px] font-bold text-slate-600 uppercase mb-1.5 ml-1 tracking-wider flex items-center gap-1">
                    ...
                    <input 
                        type="text" 
                        className="w-full p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl outline-none focus:ring-2 focus:ring-violet-500/20 text-sm font-bold dark:text-white transition-all placeholder:text-slate-600 placeholder:font-normal" 
                ...
                >
                    ...
                    <div>
                        <span className="text-sm font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2">
                            <TrendingUp size={14} className={formData.is_trending ? "text-orange-500" : "text-slate-600"}/> 
                            Trending (Топ новость)
                        </span>
                        <p className="text-[12px] text-slate-600">Показывать в блоке "Сейчас читают"</p>
                    </div>
                </div>
              </div>
              ...
              <div className="bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-5 border border-slate-100 dark:border-slate-800 h-fit">
                  <h3 className="text-xs font-black text-slate-600 uppercase tracking-widest mb-4 flex items-center gap-2"><User size={12}/> Author Details</h3>
                  
                  <div className="mb-4 relative">
                      <label className="text-[12px] font-bold text-slate-600 uppercase ml-1 mb-1 block">Select from Team</label>
                      <div className="relative">
                          <select 
                  ...
                  <div className="flex flex-col items-center mb-4">
                      ...
                      <p className="text-[12px] text-slate-600 font-bold uppercase">Author Photo</p>
                  </div>

                  <div className="space-y-3">
                      <div>
                          <label className="text-[12px] font-bold text-slate-600 uppercase ml-1">Name</label>
                      ...
                      <div>
                          <label className="text-[12px] font-bold text-slate-600 uppercase ml-1">Role</label>
                          <input className="w-full p-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-xs font-bold dark:text-white" 
          ...
          <div className="space-y-6">
              <div>
                  <label className="block text-[12px] font-bold text-slate-600 dark:text-slate-600 uppercase mb-1.5 ml-1 tracking-wider">Preview (Lead)</label>
                  <textarea 
                    className="w-full p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-sm leading-relaxed outline-none focus:ring-2 focus:ring-violet-500/20 transition-all resize-none dark:text-white placeholder:text-slate-600 h-28" 
                    value={formData.excerpt} onChange={(e) => setFormData({...formData, excerpt: e.target.value})} placeholder="Short description for the card..." 
                  />
              </div>

              <div>
                  <label className="block text-[12px] font-bold text-slate-600 dark:text-slate-600 uppercase mb-1.5 ml-1 tracking-wider">Content</label>
                  <TiptapEditor 
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
                <label className="block text-[12px] font-bold text-slate-600 uppercase tracking-wider">Cover Image</label>
            ...
            <div className="border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-2xl p-6 text-center hover:bg-slate-50 dark:hover:bg-slate-900 relative group transition-colors">
                <div className="flex gap-4 items-center">
                    ...
                    <div className="w-1/3 space-y-3">
                        ...
                        <button type="button" className="w-full p-3 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-600 flex items-center justify-center gap-2 transition pointer-events-none">
