      ...
      <div className="flex flex-col gap-1.5 w-full">
        ...
        <input
          ...
          className={clsx(
            ...
            "bg-white border placeholder:text-slate-600",
            error 
        ...
        />
        {helperText && !error && <span className="text-[12px] text-slate-600 font-medium">{helperText}</span>}
    ...
    <div className="flex flex-col gap-1.5 w-full">
      ...
      <textarea
        ...
        className={clsx(
          ...
          "bg-white border placeholder:text-slate-600 min-h-[100px]",
          error 
...
export const FormSwitch = ({ name, label }: { name: string; label: string }) => {
  ...
  return (
    <label className="flex items-center gap-3 cursor-pointer group select-none py-1">
      ...
      <span className={clsx("text-sm font-bold", checked ? "text-slate-800" : "text-slate-600")}>
        {label}
      </span>
    </label>
  );
