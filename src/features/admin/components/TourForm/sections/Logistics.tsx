  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <div className="space-y-4">
          {fields.map((field, index) => (
            <div key={field.id} className="bg-slate-50 p-5 rounded-2xl border border-slate-200 relative group transition-all hover:border-teal-200">
              ...
              <button 
                ...
                className="absolute top-4 right-4 p-2 text-slate-600 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-colors z-10"
                title="Удалить дату"
              >
                <Trash2 size={18} />
              </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4 pr-10 mb-4">
                ...
                <div className="md:col-span-4">
                   <label className="text-xs font-bold uppercase text-slate-600 mb-1.5 block">
                      Гид на выезд
                   </label>
                   <div className="relative">
                     ...
                     <User className="absolute right-3 top-3 text-slate-600 pointer-events-none" size={14} />
                   </div>
                </div>
              </div>
          ...
          {fields.length === 0 && (
            <div className="text-center py-8 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
              <p className="text-slate-600 text-sm font-medium">Даты выездов еще не добавлены</p>
              <button 
                type="button"
               onClick={() => append({ start: '', end: '', guide_id: '', groupChatUrl: '', spots: undefined, spotsLeft: undefined, basePrice: undefined, discountEarlyBird: undefined, earlyBirdDeadline: undefined, surchargeLastMinute: undefined, lastMinuteTrigger: undefined })}
        ...
        <p className="text-[12px] text-slate-600 mt-3 flex items-center gap-1">
