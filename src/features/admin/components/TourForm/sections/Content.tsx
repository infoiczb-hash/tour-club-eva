        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {highFields.map((field, index) => (
            <div key={field.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm relative group">
              ...
              <button type="button" onClick={() => removeHigh(index)} className="absolute top-2 right-2 text-slate-600 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={16}/></button>
            </div>
          ))}
          {highFields.length === 0 && <div className="col-span-full text-center py-6 border-2 border-dashed border-slate-200 rounded-xl text-slate-600 text-sm">Нет впечатлений</div>}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="p-3 bg-white border border-slate-300 rounded-xl focus-within:border-teal-500 focus-within:ring-4 focus-within:ring-teal-100 transition-all">
          ...
          <input 
            ...
            className="w-full outline-none text-sm font-medium text-slate-700 placeholder:text-slate-600"
            ...
            <div key={field.id} className="relative pl-6">
              ...
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm group">
                <div className="flex justify-between items-start mb-3">
                   ...
                   <button type="button" onClick={() => removeProg(index)} className="text-slate-600 hover:text-rose-500 mt-2"><Trash2 size={18}/></button>
                </div>
        ...
        <div className="grid gap-4">
          {faqFields.map((field, index) => (
            <div key={field.id} className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm relative group">
              ...
              <button type="button" onClick={() => removeFaq(index)} className="absolute top-2 right-2 text-slate-600 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={16}/></button>
            </div>
          ))}
        </div>
      </div>
