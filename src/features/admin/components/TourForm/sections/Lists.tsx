...
- Билеты" 
                />
                <button type="button" onClick={() => removeCheck(index)} className="absolute top-2 right-2 text-slate-800 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={16}/></button>
              </div>
            ))}
         </div>
      </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {docFields.map((field, index) => (
              <div key={field.id} className="bg-white p-4 rounded-xl border border-slate-200 relative group flex gap-3 items-start">
                ...
                <button type="button" onClick={() => removeDoc(index)} className="text-slate-800 hover:text-rose-500 transition-colors"><Trash2 size={16}/></button>
              </div>
            ))}
         </div>
      </div>
