...
interface DashboardStats {
  ...
  totalGuides: number;
  toursThisMonth: Tour[];
  allBookings: any[]; // Получаем все брони для аналитики
}
...
export default function DashboardTab({ stats, onNavigateToBookings, onEditTour }: DashboardTabProps) {
  ...
  const getTourOccupancy = (tourId: string, totalSpots: number) => {
    const tourBookings = stats.allBookings.filter(b => b.tourId === tourId);
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 cursor-pointer" onClick={onNavigateToBookings}>
        {/* Срочные чеки */}
        <div className={`rounded-2xl p-4 flex flex-col justify-between shadow-sm border transition-transform hover:scale-[1.02] ${analytics.moderation > 0 ? 'bg-orange-500 border-orange-600 text-white shadow-orange-500/20' : 'bg-white border-slate-200 text-slate-400'}`}>
        ...
        {/* Наличные (Новые) */}
        <div className={`rounded-2xl p-4 flex flex-col justify-between shadow-sm border transition-transform hover:scale-[1.02] ${analytics.pending > 0 ? 'bg-sky-500 border-sky-600 text-white shadow-sky-500/20' : 'bg-white border-slate-200 text-slate-400'}`}>
          <div className="flex justify-between items-start mb-2">
            <Users size={20} />
            <ChevronRight size={18} className="opacity-50" />
          </div>
        ...
        {/* Ждут оплаты (Онлайн) */}
        <div className={`rounded-2xl p-4 flex flex-col justify-between shadow-sm border transition-transform hover:scale-[1.02] ${analytics.awaiting > 0 ? 'bg-amber-400 border-amber-500 text-amber-900 shadow-amber-400/20' : 'bg-white border-slate-200 text-slate-400'}`}>
          <div className="flex justify-between items-start mb-2">
            <Clock size={20} />
            <ChevronRight size={18} className="opacity-50" />
          </div>
      ...
      <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mt-6">
        Общая сводка
      </h3>
      ...
      <div className="pt-4">
        ...
        {stats.toursThisMonth.length > 0 ? (
          <div className="space-y-4">
            {stats.toursThisMonth.map(tour => {
              const { green, yellow, free } = getTourOccupancy(String(tour.id), tour.spots);
              const isBurning = new Date(tour.date).getTime() - new Date().getTime() < 48 * 60 * 60 * 1000; // Менее 48 часов
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest shrink-0 overflow-x-auto pb-1 md:pb-0">
                    ...
                    <div className="flex items-center gap-1.5 text-slate-400 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-lg whitespace-nowrap">
                      Из {tour.spots} мест
                    </div>
                  </div>
                </div>
          ...
          <div className="p-10 text-center bg-white dark:bg-slate-900 rounded-3xl border border-dashed border-slate-200 dark:border-slate-800 text-slate-400 text-sm font-medium">
