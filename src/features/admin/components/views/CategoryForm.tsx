    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
      <div className="w-full max-w-lg bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex justify-between items-center p-6 border-b border-slate-800">
          ...
          <button onClick={onClose} className="text-slate-800 hover:text-white transition-colors">
            <X size={24} />
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar">
          <form id="category-form" onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1">
              <label className="text-sm font-medium text-slate-800">Название категории *</label>
              <input
                required
                name="title"
            ...
            <div className="space-y-1">
              <label className="text-sm font-medium text-slate-800">Slug (URL) *</label>
              <input
                required
                name="slug"
                value={formData.slug}
            ...
            {type === 'tour' && (
              <div className="space-y-3">
                <label className="text-sm font-medium text-slate-800">Иконка категории</label>
                      ...
                      <button
                        ...
                        className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all ${
                          isSelected
                            ...
                            : 'bg-slate-950 border-slate-800 text-slate-800 hover:border-slate-600 hover:text-slate-800'
                        }`}
                      >
                        <item.icon size={24} strokeWidth={isSelected ? 2.5 : 2} />
                        <span className="text-[12px] mt-1.5 opacity-70 truncate w-full text-center">
                          {item.name}
                        </span>
                      </button>
                    );
            ...
            {type === 'tour' && (
              <div className="space-y-3 border-t border-slate-800 pt-4">
                <label className="text-sm font-medium text-slate-800">Фирменный цвет категории</label>
            ...
            <div className="grid grid-cols-2 gap-4 border-t border-slate-800 pt-4">
              <div className="space-y-1">
                <label className="text-sm font-medium text-slate-800">Порядок сортировки</label>
                <input
                  type="number"
                  name="sort_order"
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-white focus:border-teal-500 outline-none"
                />
                <p className="text-[11px] text-slate-800">Меньше цифра = выше в списке</p>
              </div>
              ...
              <div className="flex items-center mt-6">
                <label className="flex items-center gap-3 cursor-pointer group">
                  ...
                  <span className="text-sm font-medium text-slate-800 group-hover:text-white transition-colors">
                    Активна
                  </span>
                </label>
              </div>
