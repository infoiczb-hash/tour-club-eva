...
const CATEGORY_MAP: Record<string, { label: string, colorClass: string }> = {
  general: { label: 'Местное', colorClass: 'bg-slate-100 text-slate-800 border-slate-200 dark:bg-slate-800 dark:text-slate-800 dark:border-slate-700' },
    ...
    <div className="space-y-6">
      ...
      <div className="hidden md:block bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
        <table className="w-full text-sm text-left">
          <thead className="bg-slate-50 dark:bg-slate-800/50 text-slate-800 font-black uppercase text-[12px] tracking-widest">
            <tr>
                ...
                <tr key={review.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors duration-200">
                  <td className="p-5 font-bold text-slate-800 dark:text-white">
                    <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-800 flex items-center justify-center text-xs font-black text-slate-800 shrink-0 uppercase">
                            {review.name[0]}
                        </div>
                  ...
                  <td className="p-5">
                    <div className="flex items-center gap-2 border border-slate-200 dark:border-slate-700 rounded-lg px-2.5 py-1 w-fit bg-slate-50 dark:bg-slate-800/50">
                      ...
                      <span className="text-[12px] font-bold uppercase text-slate-800">{review.source}</span>
                    </div>
                  </td>
                  <td className="p-5">
                  ...
                  <td className="p-5 text-slate-800 dark:text-slate-800 whitespace-pre-wrap leading-relaxed text-xs">
                  ...
                  <td className="p-5 text-center">
                    ...
                    <button 
                      ...
                      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[12px] font-bold uppercase transition-all ${
                        ...
                        : 'bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700'
                      }`}
                    >
                  ...
                  <td className="p-5 text-right">
                    ...
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <button onClick={() => onEdit(review)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg text-slate-800 hover:text-teal-500 transition-colors">
                        <Edit size={16}/>
                      </button>
                      <button onClick={() => onDelete(review.id)} className="p-2 hover:bg-red-50 dark:hover:bg-red-500/10 rounded-lg text-slate-800 hover:text-red-500 transition-colors">
                        <Trash2 size={16}/>
                      </button>
                    </div>
                  </td>
           ...
           return (
            <div key={review.id} className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm relative flex flex-col">
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center font-black text-slate-800 uppercase shrink-0">
                          {review.name[0]}
                      </div>
                      <div>
                          <h4 className="font-bold text-slate-900 dark:text-white text-sm">{review.name}</h4>
                          <div className="flex items-center gap-2 mt-1">
                              <span className="flex items-center gap-1 text-[9px] text-slate-800 uppercase font-bold">
                  ...
                  <button onClick={() => onToggleStatus(review)} className={`p-1 transition-colors ${review.isActive ? 'text-emerald-500' : 'text-slate-800 dark:text-slate-800'}`}>
                    {review.isActive ? <CheckCircle2 size={20}/> : <Circle size={20}/>}
                  </button>
                </div>
                
                <p className="text-sm text-slate-800 dark:text-slate-800 mb-4 bg-slate-50 dark:bg-slate-800/50 p-3.5 rounded-xl italic leading-relaxed">
                    "{review.text}"
                </p>
                
                <div className="flex justify-end gap-2 border-t border-slate-100 dark:border-slate-800 pt-3">
                    <button onClick={() => onEdit(review)} className="px-4 py-2 bg-slate-100 dark:bg-slate-800 rounded-xl text-slate-800 dark:text-slate-800 text-xs font-bold flex items-center gap-2 active:scale-95 transition-transform">
