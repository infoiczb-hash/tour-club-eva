...
const STATUS_LABELS = {
  ...
  DONE: { label: 'Архив', color: 'bg-slate-500/10 text-slate-800 border-slate-500/20' },
};
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between gap-4 md:items-center">
         <h1 className="text-2xl font-black uppercase tracking-tight dark:text-white flex items-center gap-3">
            ...
            <span className="text-sm bg-slate-100 dark:bg-slate-800 text-slate-800 px-2 py-1 rounded-lg">
             ...
             <div className="flex bg-white dark:bg-slate-900 p-1 rounded-xl border border-slate-200 dark:border-slate-800">
                 {(['ALL', 'NEW', 'WORK', 'DONE'] as const).map(s => (
                    <button 
                       ...
                       className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${filterStatus === s ? 'bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white' : 'text-slate-800 hover:text-slate-800'}`}
                    >
      ...
      <div className="grid gap-3">
         {filtered.length === 0 && (
             <div className="text-center py-20 text-slate-800">Нет обращений по выбранным фильтрам</div>
         )}
               ...
               <div key={item.id} className={`group relative bg-white dark:bg-slate-900 p-5 rounded-2xl border transition-all ${item.status === 'NEW' ? 'border-teal-500/40 shadow-sm' : 'border-slate-200 dark:border-slate-800 opacity-80 hover:opacity-100'}`}>
                  ...
                  <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-2">
                          ...
                          <div>
                              <span className="text-[12px] font-bold text-slate-800 block uppercase tracking-wider">{item.type}</span>
                  ...
                  <div className="grid md:grid-cols-4 gap-4 items-start">
                      ...
                      <div className="md:col-span-2">
                          ...
                          <p className="text-sm text-slate-800 dark:text-slate-800 whitespace-pre-wrap leading-relaxed">
                              {item.message}
                          </p>
                      </div>
