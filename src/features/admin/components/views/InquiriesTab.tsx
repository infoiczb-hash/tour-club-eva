  ...
  return (
    <div className="space-y-6">
             ...
             <div className="flex bg-white dark:bg-slate-900 p-1 rounded-xl border border-slate-200 dark:border-slate-800">
                 {(['ALL', 'NEW', 'WORK', 'DONE'] as const).map(s => (
                    <button 
                       ...
                       className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${filterStatus === s ? 'bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white' : 'text-slate-400 hover:text-slate-600'}`}
                    >
      ...
      <div className="grid gap-3">
         {filtered.length === 0 && (
             <div className="text-center py-20 text-slate-400">Нет обращений по выбранным фильтрам</div>
         )}
               ...
               <div key={item.id} className={`group relative bg-white dark:bg-slate-900 p-5 rounded-2xl border transition-all ${item.status === 'NEW' ? 'border-teal-500/40 shadow-sm' : 'border-slate-200 dark:border-slate-800 opacity-80 hover:opacity-100'}`}>
                  ...
                  <div className="flex justify-between items-start mb-3">
                      <div className="flex items-center gap-2">
                          ...
                          <div>
                              <span className="text-[10px] font-bold text-slate-400 block uppercase tracking-wider">{item.type}</span>
                      ...
                      <div className="flex items-center gap-1 bg-slate-50 dark:bg-slate-800 p-1 rounded-lg">
                          <button onClick={() => handleStatus(item.id, 'NEW')} title="Новое" className={`p-1.5 rounded ${item.status === 'NEW' ? 'bg-emerald-500 text-white shadow-sm' : 'text-slate-400 hover:bg-slate-200'}`}><Clock size={14}/></button>
                          <button onClick={() => handleStatus(item.id, 'WORK')} title="В работе" className={`p-1.5 rounded ${item.status === 'WORK' ? 'bg-blue-500 text-white shadow-sm' : 'text-slate-400 hover:bg-slate-200'}`}><Eye size={14}/></button>
                          <button onClick={() => handleStatus(item.id, 'DONE')} title="Архив" className={`p-1.5 rounded ${item.status === 'DONE' ? 'bg-slate-500 text-white shadow-sm' : 'text-slate-400 hover:bg-slate-200'}`}><Archive size={14}/></button>
                      </div>
                  </div>
