      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex bg-slate-200/50 dark:bg-slate-800/50 p-1.5 rounded-2xl w-fit">
            <button 
                ...
                className={clsx("flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold uppercase tracking-wider transition-all", activeMode === 'list' ? 'bg-white text-teal-600 shadow-sm' : 'text-slate-500 hover:text-slate-700')}
            >Лента</button>
            <button 
                onClick={() => setActiveMode('groups')}
                className={clsx("flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold uppercase tracking-wider transition-all", activeMode === 'groups' ? 'bg-white text-teal-600 shadow-sm' : 'text-slate-500 hover:text-slate-700')}
            >Списки групп</button>
        </div>
      </div>
                              ...
                              <React.Fragment key={b.id}>
                                <tr className="group hover:bg-slate-50 transition-colors">
                                    <td className="p-5 align-top">
                                        ...
                                        <div className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-slate-100 rounded text-[10px] font-mono text-slate-500">
                                    ...
                                    <td className="p-5 align-top">
                                        ...
                                        <div className="flex flex-col gap-1">
                                            <span className="text-xs text-slate-500 font-mono"><Phone size={10} className="inline"/> {b.user_phone}</span>
                                        </div>
                                    </td>
                                    <td className="p-5 align-top">
                                    ...
                                    <td className="p-5 align-top">
                                        {b.comment ? <div className="text-xs text-slate-600 border-l-2 border-amber-500/50 pl-2">{b.comment}</div> : <span className="text-slate-300">—</span>}
                                    </td>
                                    <td className="p-5 text-center align-top">
                                        <select 
                                            value={b.status} onChange={(e) => onStatusChange(b.id, e.target.value)} 
                                            className={clsx("text-xs font-bold uppercase tracking-wider rounded-lg px-3 py-1.5 outline-none cursor-pointer border text-center w-full", b.status === 'confirmed' ? "bg-emerald-50 text-emerald-600 border-emerald-200" : "bg-amber-50 text-amber-600 border-amber-200")}
                                        >
                                    ...
                                    <td colSpan={6} className="p-4 px-8">
                                      <div className="flex flex-wrap gap-2">
                                        <div className="px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs flex flex-col gap-0.5 shadow-sm">
                                          ...
                                          <span className="text-slate-500 text-[10px] uppercase">Взрослый</span>
                                        </div>
                                        {guests.map((g, i) => (
                    ...
                    <div key={b.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden flex flex-col gap-4 p-5">
                            ...
                            <div className="flex justify-between items-start">
                                <div>
                                    <div className="font-black text-lg text-slate-900 flex items-center gap-2">
                                        {b.user_name} <span className="text-slate-400 text-xs font-mono font-normal">#{b.short_id || '---'}</span>
                                    </div>
                                ...
                                <div className="pt-3 border-t border-slate-200/60 flex flex-col gap-2">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        <div className="text-xs flex items-center justify-between">
                                            ...
                                            <span className="text-[10px] text-slate-500 uppercase">Взрослый</span>
                                        </div>
                                        {guests.map((g, i) => (
                                            <div key={i} className="text-xs flex items-center justify-between">
                                                ...
                                                <div className="flex items-center gap-1.5">
                                                    ...
                                                    <span className="text-[10px] text-slate-500 uppercase">{getTicketLabel(g.ticketType, g.age)}</span>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                        ...
                        {b.comment && (
                            <div className="ml-2 bg-amber-500/10 border border-amber-500/20 rounded-xl p-3 flex gap-3 items-start">
                                ...
                                <p className="text-xs font-medium text-amber-700 leading-relaxed">{b.comment}</p>
                            </div>
                        )}
      ...
      {activeMode === 'groups' && (
        <div className="space-y-4 animate-in slide-in-from-right-4 duration-300">
            {groupedManifests.length === 0 ? (
                <div className="p-10 text-center text-slate-500 font-medium bg-white rounded-3xl border border-dashed border-slate-200">Активных выездов пока нет</div>
            ) : (
                            ...
                            {isOpen && (
                                <div className="border-t border-slate-100 bg-slate-50/30 overflow-x-auto">
                                    <table className="w-full text-left text-sm whitespace-nowrap">
                                      <thead className="bg-slate-100/50 text-slate-500 text-[10px] font-black uppercase tracking-widest border-b border-slate-200">
                                        <tr>
                                      ...
                                      <tbody className="divide-y divide-slate-100">
                                        {group.participants.map((p, index) => (
                                          <tr key={`${p.bookingId}-${index}`} className={clsx(
                                            ...
                                            <td className="px-6 py-4 text-xs uppercase font-bold text-slate-500 tracking-wider">
                                            ...
                                            <td className="px-6 py-4 text-xs text-slate-500 whitespace-normal">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl">
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
              <button 
                onClick={() => setBroadcastModal({isOpen: false, group: null})}
                className="px-6 py-3 font-bold text-slate-500 text-xs uppercase tracking-widest hover:text-slate-700 transition-colors"
              >
                Отмена
              </button>
              <button 
