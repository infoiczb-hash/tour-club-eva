          ...
          <div className="hidden md:block bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                    ...
                    <tr key={post.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition">
                      <td className="p-5">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <div className="text-[12px] text-slate-600 font-mono">{new Date(post.date).toLocaleDateString()}</div>
                          </div>
                        </div>
                      </td>
                      <td className="p-5">
                      ...
                      <td className="p-5 text-center">
                        <button 
                          ...
                          className={`p-1.5 rounded-lg transition ${post.is_trending ? 'text-amber-500 bg-amber-50 dark:bg-amber-900/20' : 'text-slate-600 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
                        >
                ...
                {posts.length === 0 && (
                  <tr>
                    <td colSpan={6} className="p-10 text-center text-slate-600 font-medium">Статей пока нет</td>
                  </tr>
                )}
              </tbody>
            </table>
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-between">
                      ...
                      <div className="flex justify-end gap-2 mt-2">
                        <button onClick={() => onToggleStatus(post, 'is_trending')} className={`p-2 rounded-lg ${post.is_trending ? 'text-amber-500 bg-amber-50 dark:bg-amber-900/20' : 'text-slate-600 bg-slate-50 dark:bg-slate-800'}`}><Star size={16}/></button>
                        <button onClick={() => onEdit(post)} className="p-2 bg-slate-100 dark:bg-slate-800 rounded-lg text-slate-600"><Edit size={16}/></button>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
              <table className="w-full text-sm text-left">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      ...
                      {categories.length === 0 && (
                          <tr>
                              <td colSpan={5} className="p-10 text-center text-slate-600 font-medium">Категории блога не найдены</td>
                          </tr>
                      )}
                  </tbody>
              </table>
