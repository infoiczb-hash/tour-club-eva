    ...
    <div className="space-y-6 animate-in fade-in duration-300">
        ...
        {activeView === 'tours' && (
          <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
            <div className="bg-white dark:bg-slate-900 p-1.5 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col md:flex-row gap-2 shadow-sm">
                ...
                <div className="flex-1 relative">
                    ...
                    <input 
                      ...
                      className="w-full h-full pl-9 pr-4 bg-transparent text-sm font-medium outline-none text-slate-900 dark:text-white placeholder:text-slate-800" 
                      value={searchTerm} 
                ...
                <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            ...
                            return (
                                <tr key={tour.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                    <td className="p-5">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="flex flex-col">
                                                ...
                                                >
                                                    {tour.title}
                                                    <ExternalLink size={10} className="opacity-0 group-hover/link:opacity-100 transition-opacity text-slate-800"/>
                                                </Link>
                                    ...
                                    <td className="p-5 font-medium">
                                        <div className="text-slate-900 dark:text-slate-800">{new Date(tour.date).toLocaleDateString()}</div>
                                        <div className="text-xs text-slate-800 font-bold">{tour.duration} дн.</div>
                                     </td>
                                    <td className="p-5">
                                        <div className="flex justify-between text-[12px] font-black mb-1.5">
                                            <span className={percent >= 100 ? 'text-rose-500' : 'text-slate-800 dark:text-slate-800'}>{booked} / {tour.spots}</span>
                                            <span className="text-slate-800 dark:text-slate-800">{Math.round(percent)}%</span>
                                        </div>
                                    ...
                                    <td className="p-5 font-black text-slate-900 dark:text-white">{tour.price} <span className="text-xs text-slate-800 font-bold">{tour.currency}</span></td>
                        ...
                        {filteredTours.length === 0 && (
                            <tr>
                                <td colSpan={6} className="p-10 text-center text-slate-800 font-medium">Туров не найдено</td>
                            </tr>
                        )}
                    </tbody>
                </table>
                     ...
                     <div key={tour.id} className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
                         <div className="flex gap-4 mb-3">
                             ...
                             <div className="flex-1 min-w-0">
                                 ...
                                 <p className="text-xs text-slate-800 font-medium mb-2">{new Date(tour.date).toLocaleDateString()} • {tour.duration} дн.</p>
            ...
            {loading && (
              <div className="p-10 text-center text-slate-700">Загрузка туров...</div>
            )}
            {!loading && (
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {categories.map((cat) => (
                            <tr key={cat.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                <td className="p-5">
                                    <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-800 dark:text-slate-800">
                                        <LayoutGrid size={20} />
                                    </div>
                                </td>
                        ...
                        {categories.length === 0 && (
                            <tr>
                                <td colSpan={6} className="p-10 text-center text-slate-800 font-medium">Категории не найдены</td>
                            </tr>
                        )}
                    </tbody>
                </table>
