  ...
  if (initialTests.length === 0) {
    return (
      <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center shadow-sm">
        ...
        <p className="text-slate-600 mb-6">Создай первую карточку, чтобы она появилась в Фан-секторе.</p>
      </div>
    );
  }
    ...
    <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 text-xs uppercase tracking-widest font-bold">
            ...
            {initialTests.map((test) => (
              <tr key={test.id} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="p-5">
                  ...
                  <div className="text-xs font-mono text-slate-600 mt-1 bg-slate-100 px-2 py-0.5 rounded w-fit">
                    {test.slug}
                  </div>
                </td>
                ...
                <td className="p-5 text-center">
                  ...
                  <div className="text-[12px] font-bold uppercase tracking-widest mt-2 text-slate-600">
                    {test.isActive ? (
                ...
                <td className="p-5 text-right">
                  ...
                  <button
                    onClick={() => onEdit(test)}
                    className="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-white border border-slate-200 text-slate-600 hover:text-indigo-600 hover:border-indigo-200 hover:bg-indigo-50 transition-all shadow-sm"
                  >
                    <Edit size={16} />
                  </button>
                </td>
