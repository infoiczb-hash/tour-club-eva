...
export default function Hero() {
  ...
  useEffect(() => {
    let rafId: number;
    let lastScrollY = 0;
    let ticking = false; // ← RAF запускается только при реальном скролле
    ...
    const onScroll = () => {
      ...
      if (!ticking) {
        rafId = requestAnimationFrame(() => {
          ...
          setContentOpacity(Math.max(0, 1 - lastScrollY / 400));
          ticking = false;
        });
        ticking = true;
      }
  ...
  return (
    <section className="relative min-h-[100svh] w-full overflow-hidden bg-slate-950 flex flex-col items-center justify-center">
      ...
      >
        ...
        {/* ПЕРЕКЛЮЧАТЕЛЬ — layoutId не тронут */}
        <div className="opacity-0 animate-fade-in-up [animation-delay:450ms] bg-slate-900/80 p-1.5 rounded-2xl border border-white/10 inline-flex flex-col sm:flex-row gap-1 w-full sm:w-auto shadow-2xl backdrop-blur-md">
          ...
          >
            {activeTab === 'newbie' && (
              <m.div layoutId="heroTab" className="absolute inset-0 bg-teal-500 rounded-xl shadow-[0_0_20px_rgba(20,184,166,0.4)]" />
            )}
      ...
      {/* SCROLL INDICATOR — CSS анимация вместо framer-motion */}
      <div className="opacity-0 animate-fade-in-up [animation-delay:1000ms] absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 pointer-events-none">
        ...
        <div className="w-6 h-10 border-2 border-slate-500 rounded-full flex justify-center p-1">
          <div className="w-1 h-2 bg-teal-500 rounded-full animate-[scroll-dot_1.5s_ease-in-out_infinite]" />
        </div>
      </div>
    </section>
  );
