...
import { Anchor, XCircle, ShieldAlert } from "lucide-react";

// --- DUMMY DATA FOR RULES (Ваш оригинальный текст) ---
const rules = [
  ...
  return (
    <section className="py-12 md:py-20 bg-[#020617] text-slate-200 overflow-hidden font-sans border-t border-white/5 relative">
      ...
      <div className="container mx-auto px-4 max-w-6xl relative z-10">
        ...
        <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 pb-8 -mx-4 px-4 md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-6 md:mx-0 md:px-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            {rules.map((rule, idx) => (
                <motion.div 
                    key={rule.id} 
        ...
        <motion.div 
            ...
            viewport={{ once: true }}
            className="mt-4 p-6 md:p-8 rounded-[2.5rem] bg-gradient-to-r from-teal-900/40 to-slate-900 border border-teal-500/20 flex flex-col md:flex-row items-center gap-6 md:gap-8"
        >
