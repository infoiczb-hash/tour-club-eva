...
export default function PopularRoutes() {
  ...
  const nextImage = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (selectedRoute) {
      setCurrentImgIdx((prev) => (prev + 1) % selectedRoute.images.length);
    }
  };
  ...
  const prevImage = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (selectedRoute) {
      setCurrentImgIdx((prev) => (prev - 1 + selectedRoute.images.length) % selectedRoute.images.length);
    }
  };
        ...
        {selectedRoute && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-6 bg-slate-950/90 backdrop-blur-xl">
            ...
            >
              ...
              {/* ЛЕВАЯ ЧАСТЬ: Карусель фотографий */}
              ...
              {/* ПРАВАЯ ЧАСТЬ: Информация и Паспорт */}
