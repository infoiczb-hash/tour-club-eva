...
export default function OrgContact() {
  ...
  const { ref: refCard1, inView: card1InView } = useInView();
  const { ref: refCard2, inView: card2InView } = useInView();
    ...
    <section className="py-12 md:py-20 bg-slate-950 border-t border-white/5 relative overflow-hidden">
      <div className="container mx-auto px-4 max-w-5xl relative z-10">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          {/* КАРТОЧКА 1: Официальная заявка */}
          <div
            ref={refCard1}
            className={cn(
              'p-8 md:p-10 bg-slate-900/60 rounded-[2rem] border border-white/10 hover:border-indigo-500/40 transition-all duration-500 ease-out flex flex-col justify-between group',
              card1InView ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'
            )}
          >
            <div>
          ...
          {/* КАРТОЧКА 2: Прямая связь в Telegram */}
          <div
            ref={refCard2}
            className={cn(
              'p-8 md:p-10 bg-gradient-to-br from-[#229ED9]/10 to-slate-900/60 rounded-[2rem] border border-[#229ED9]/20 hover:border-[#229ED9]/50 transition-all duration-500 ease-out flex flex-col justify-between group relative overflow-hidden',
              card2InView ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'
            )}
          >
