...
import { Gamepad2, Backpack, Compass, ArrowRight, Trophy, Sparkles, Shield, Dumbbell, Activity, BookOpen } from "lucide-react";
...
// 2. РЕЕСТР ВИЗУАЛА (Привязываем красивые цвета и иконки по slug)
const VISUAL_REGISTRY: Record<string, { color: string; icon: React.ReactNode }> = {
  ...
  // Дефолтный стиль, если slug не найден в словаре визуалов
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-slate-200 selection:bg-indigo-500/30 overflow-hidden relative">
      ...
      <section className="container mx-auto px-4 pb-24 relative z-10">
         <motion.div variants={containerVariants} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
            ...
            {/* CTA BANNER */}
            <motion.div variants={itemVariants} className="md:col-span-2 lg:col-span-3 bg-gradient-to-r from-teal-900/40 to-slate-900 border border-white/5 rounded-[2.5rem] p-8 md:p-12 relative overflow-hidden group">
               ...
               <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6 text-center md:text-left">
                  <div>
                     ...
                     <p className="text-slate-400 max-w-lg text-lg">Теория и психология — это отлично. Но настоящие ответы ждут тебя на маршруте.</p>
                  </div>
                  <Link href="/tour" className="px-8 py-4 bg-teal-500 text-slate-950 font-black uppercase tracking-wider rounded-2xl hover:bg-teal-400 transition-all shadow-[0_0_20px_rgba(20,184,166,0.3)] whitespace-nowrap">
                     Смотреть все туры
                  </Link>
               </div>
            </motion.div>
      ...
      {/* --- ДИНАМИЧЕСКИЕ МОДАЛКИ ИЗ РЕЕСТРА --- */}
      ...
      <ContactHubModal isOpen={isHubOpen} onClose={() => setIsHubOpen(false)} initialTab="TOUR" />
    </main>
  );
}
...
function QuizCard({ onClick, image, color, icon, title, desc, category }: any) {
    ...
    return (
        <motion.div variants={itemVariants} whileHover={{ y: -8 }} onClick={onClick} className={clsx("group relative h-[380px] bg-slate-900 rounded-[2.5rem] overflow-hidden border border-white/5 cursor-pointer transition-all shadow-2xl", colors[color].split(" ").pop())}>
           {image && (
             <Image src={image} alt={title} fill className="object-cover opacity-50 grayscale-[30%] group-hover:grayscale-0 group-hover:scale-105 transition-transform duration-700" sizes="(max-width: 768px) 100vw, 33vw" />
           )}
           ...
           <div className="absolute inset-0 p-8 flex flex-col justify-end">
              ...
              {/* Рендерим заголовок, разбивая по пробелам (если нужно) или просто как текст */}
