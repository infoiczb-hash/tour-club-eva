...
export default function FullProfileModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div 
      className={`fixed inset-0 z-[110] flex items-center justify-center bg-slate-950/95 backdrop-blur-2xl px-4 transition-opacity duration-200 ease-out ${closing ? "opacity-0" : "opacity-100"}`}
    >
      <div 
        role="dialog" 
        aria-modal="true" 
        aria-labelledby="modal-fullprofile-title" 
        className={`relative w-full max-w-3xl bg-slate-900/50 border border-indigo-500/20 rounded-[3rem] p-8 md:p-12 shadow-[0_0_50px_rgba(99,102,241,0.1)] overflow-y-auto max-h-[90vh] custom-scrollbar transition-all duration-200 ease-out ${closing ? "scale-95 opacity-0 translate-y-4" : "scale-100 opacity-100 translate-y-0"}`}
      >
