...
import { ArrowRight, Gamepad2, Compass, Flame, Backpack, Shield, Dumbbell, Activity, BookOpen, Brain, Heart, Search, Users, Ghost, Sparkles } from 'lucide-react';
import { clsx } from 'clsx';
import { twMerge } from "tailwind-merge";

// Импортируем наш экшен для получения данных
...
// --- РЕЕСТР ВИЗУАЛА (Как на главной странице Fun-сектора) ---
...
// Дефолтные тесты, если база пока пустая (или для первого рендера)
...
export default function FunSectorWidget() {
  ...
  useEffect(() => {
    ...
    const fetchAndShuffleQuizzes = async () => {
      try {
        ...
        // 1. Группируем тесты по категориям
        ...
        // 2. Берем случайные категории
        const shuffledCategories = Object.keys(grouped).sort(() => 0.5 - Math.random());
        let selectedTests: FunTest[] = [];

        // 3. Берем по 1 случайному тесту из первых 3-х перемешанных категорий
        for (let i = 0; i < Math.min(3, shuffledCategories.length); i++) {
            const catTests = grouped[shuffledCategories[i]];
            const randomTest = catTests[Math.floor(Math.random() * catTests.length)];
            selectedTests.push(randomTest);
        }

        // 4. ЗАЩИТА: Если категорий меньше 3-х, но тестов много — добиваем до 3-х штук
        if (selectedTests.length < 3 && activeTests.length > selectedTests.length) {
            const remainingTests = activeTests.filter(t => !selectedTests.find(s => s.id === t.id));
            remainingTests.sort(() => 0.5 - Math.random()); // Перемешиваем остатки
            const needed = 3 - selectedTests.length;
            selectedTests = [...selectedTests, ...remainingTests.slice(0, needed)];
        }

        if (isMounted) {
            setQuizzes(selectedTests);
            setIsLoaded(true);
        }
      } catch (error) {
  ...
  return (
    <section className="py-12 md:py-20 bg-slate-950 relative overflow-hidden border-t border-white/5">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px] bg-violet-900/10 md:md:blur-[120px] rounded-full pointer-events-none" />
      ...
      <div className="container mx-auto px-4 max-w-7xl relative z-10">
        ...
        {/* СЕТКА С АНИМАЦИЕЙ (AnimatePresence позволяет плавно заменить Fallback на Реальные данные) */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 md:gap-4 min-h-[100px] lg:min-h-[200px]">
            <AnimatePresence mode="popLayout">
                {quizzes.map((quiz, idx) => (
                    <motion.div 
                        key={`${quiz.id}-${isLoaded ? 'loaded' : 'initial'}`} // Трюк для триггера анимации при смене
...
// ОБНОВЛЕННАЯ КАРТОЧКА, ПРИНИМАЮЩАЯ ДАННЫЕ ИЗ БАЗЫ
        ...
        >
            ...
            <div className="relative z-10 flex w-full h-full flex-row lg:flex-col items-center lg:items-start">
                ...
                <div className="flex-1 lg:mt-6 w-full">
                    {/* Категория на десктопе */}
                    <div className="hidden lg:block text-[9px] font-bold uppercase tracking-widest text-slate-400 mb-2">
                        {quiz.category}
                    </div>
