...
import { useModalTransition } from "@/hooks/useModalTransition"; // <-- Наш хук
...
export default function PhysicalReadinessModal({ isOpen, onClose }: Props) {
  ...
  // Подключаем наш хук для анимации закрытия
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* === 1. ИНТРО === */}
        {step === "intro" && (
          <div key="intro" className="flex flex-col h-full overflow-hidden p-6 md:p-10 pb-6 text-center justify-center animate-in fade-in zoom-in-95 duration-300">
        ...
        {/* === 2. ВОПРОСЫ === */}
        {step === "questions" && (
          <div key="questions" className="flex flex-col h-full overflow-hidden p-6 md:p-10 pb-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 pb-4 space-y-3">
              {currentQuestion.options.map((opt) => (
                  <button key={opt.value} onClick={() => handleAnswer(opt.value)} className="w-full text-left px-5 py-4 rounded-2xl border border-white/5 bg-slate-800/50 hover:bg-slate-800 hover:border-cyan-500/50 transition-all duration-300 flex items-center gap-4 group active:scale-[0.98]">
        ...
        {/* === 3. САММАРИ === */}
        {step === "summary" && (
          <div key="summary" className="flex flex-col h-full overflow-hidden p-6 md:p-10 pb-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
                      ...
                      <div className="pt-8 mt-6 border-t border-white/10 text-center animate-in fade-in duration-500">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                          ...
                          <Link
                            href="/tour"
                            onClick={onClose}
                            className="flex-1 py-4 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-bold text-[11px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-lg shadow-cyan-900/20 active:scale-95"
                          >
