...
import { useModalTransition } from "@/hooks/useModalTransition"; // <-- Наш новый хук
...
export default function FearDebriefModal({ isOpen, onClose }: Props) {
  ...
  // Подключаем хук анимации закрытия
              ...
              {FEARS.map((fear) => {
                ...
                return (
                  <div key={fear.key} className="flex gap-3 items-stretch">
                      ...
                      <button onClick={() => toggleFear(fear.key)} className={cn("flex-1 text-left px-5 py-4 rounded-2xl border transition-all duration-300 flex items-center justify-between gap-4 group", isSel ? "border-blue-500/50 bg-blue-500/10 shadow-[0_0_15px_rgba(59,130,246,0.15)] transform scale-[1.02]" : "border-white/5 bg-slate-800/50 hover:bg-slate-800 hover:scale-[1.01] active:scale-95")}>
                        <div>
                    ...
                    <div className="pt-8 mt-6 border-t border-white/10 text-center animate-in fade-in duration-500">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        <Link
                          href="/tour"
                          onClick={onClose}
                          className="flex-1 py-4 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-[11px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-900/20 active:scale-95"
                        >
