...
function getStatusBadge(status: string) {
  ...
  const baseClasses = "px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border";
  
  if (s === "pending") return <span className={`${baseClasses} bg-amber-500/10 text-amber-400 border-amber-500/20`}>Новая (Наличные)</span>;
  if (s === "cancelled") return <span className={`${baseClasses} bg-slate-500/10 text-slate-400 border-slate-500/20`}>Отменено</span>;
...
export default async function BookingDetailsPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="max-w-2xl mx-auto pb-12 animate-in fade-in duration-500 px-4">
      ...
      <Link href="/account" className="inline-flex items-center gap-2 text-slate-400 hover:text-white mb-6 text-sm font-bold uppercase tracking-widest transition-colors">
      ...
      <div className="space-y-6">
          ...
          <div className="p-6 sm:p-8 border-b border-white/5 border-dashed relative">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-slate-950/50 rounded-2xl p-4 border border-white/5">
                ...
                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest mb-1">Дата и Время старта</p>
                <p className="text-sm font-bold text-white">{dateStr} в {timeStr}</p>
              </div>
              <div className="bg-slate-950/50 rounded-2xl p-4 border border-white/5">
                <MapPin className="text-teal-500 mb-2" size={18} />
                <p className="text-[10px] text-slate-500 uppercase font-bold tracking-widest mb-1">Точка сбора</p>
          ...
          <div className="p-6 sm:p-8 bg-gradient-to-b from-slate-900 to-slate-950 space-y-8">
            ...
            {showChatButton && (
              <div className="animate-in slide-in-from-bottom-4 duration-700">
                ...
                <p className="text-center text-[10px] text-slate-500 uppercase font-bold mt-3 tracking-wider">
                ...
                {guests.length > 0 ? guests.map((guest, idx) => (
                  <div key={idx} className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/5">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-slate-400 font-black text-xs">{idx + 1}</div>
                      <p className="text-sm font-bold text-white">{guest.name} <span className="text-[10px] text-slate-500 uppercase ml-2">{guest.ticketType}</span></p>
                    </div>
                    {guest.equipment && (
                  ...
                  <div className="p-4 rounded-2xl bg-white/5 border border-white/5 flex items-center gap-3">
                    ...
                    <p className="text-sm font-bold text-white">{booking.name} <span className="text-[10px] text-slate-500 uppercase ml-2">Заказчик</span></p>
                  </div>
                )}
              </div>
            </div>
                ...
                <div className="bg-slate-950/40 rounded-2xl p-5 border border-white/5 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {checklist.map((block: any, i: number) => (
                    <div key={i} className="space-y-1">
                      <p className="text-[10px] font-black text-teal-500 uppercase tracking-wider">{block.title}</p>
                      <p className="text-xs text-slate-400 leading-relaxed">{block.items}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div>
              ...
              <div className="bg-slate-950 rounded-2xl p-6 border border-white/5 space-y-4 mb-6">
                <div className="flex justify-between text-sm"><span className="text-slate-500">Стоимость:</span><span className="text-white font-bold">{booking.totalPrice} {booking.tour.currency}</span></div>
                ...
                <div className="flex justify-between text-sm"><span className="text-slate-500">Оплачено:</span><span className="text-white font-bold">{booking.amountPaid} {booking.tour.currency}</span></div>
        ...
        <div className="bg-slate-900/50 border border-white/5 rounded-3xl p-6 flex flex-col sm:flex-row items-center justify-between gap-6 mt-4">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-slate-800 flex items-center justify-center text-slate-400"><AlertTriangle size={20} /></div>
            <div>
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Нужна помощь?</p>
              <p className="text-sm font-bold text-white">Служба заботы Турклуба</p>
            </div>
          </div>
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <a href="tel:+37377770141" className="flex-1 sm:flex-none px-6 py-3 bg-white/5 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all hover:bg-white/10 flex items-center justify-center gap-2">
              <Phone size={14} /> Звонок
            </a>
            <a href="https://t.me/romansvtirase" target="_blank" rel="noreferrer" className="flex-1 sm:flex-none px-6 py-3 bg-[#2AABEE]/10 text-[#2AABEE] rounded-xl text-[10px] font-black uppercase tracking-widest transition-all hover:bg-[#2AABEE]/20 flex items-center justify-center gap-2">
