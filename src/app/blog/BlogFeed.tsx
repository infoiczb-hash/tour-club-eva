...
// 🔥 ИСПРАВЛЕНИЕ TBT: Карточка вынесена за пределы BlogFeed, чтобы React не уничтожал DOM
const PostCard = ({ 
  post, 
  categories = [], 
  priority = false 
  ...
  return (
    // 🔥 ИСПРАВЛЕНИЕ UI: border-white/20 для мобилок делает контур заметнее, md:border-white/5 оставляет аккуратным на ПК
    <div className="group flex flex-col bg-slate-900/60 border border-white/20 md:border-white/5 rounded-[2rem] overflow-hidden hover:bg-slate-800/80 hover:border-teal-500/40 md:hover:border-teal-500/30 transition-all duration-300 shadow-md md:shadow-sm hover:shadow-xl h-full">
      
      {/* 🔥 ИСПРАВЛЕНИЕ LCP и СЕТИ: Отключаем автоматический RSC-prefetch для ссылок */}
      <Link href={`/blog/${post.slug}`} prefetch={false} className="relative aspect-[16/10] w-full overflow-hidden bg-slate-800 block">
        <Image
          ...
          // 🔥 ИСПРАВЛЕНИЕ LCP: sizes="(max-width: 640px) 80vw..." заставит мобилки качать легкий w_640
      ...
      <div className="p-6 md:p-8 flex flex-col flex-grow">
        <div className="flex items-center justify-between text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-4">
          {/* Читаем готовую строку с сервера */}
          <span>{post.precalculatedDate}</span>
        ...
        {/* Заголовок */}
        {/* 🔥 ИСПРАВЛЕНО: Добавлен prefetch={false} */}
        <Link href={`/blog/${post.slug}`} prefetch={false} className="block mb-3">
        ...
        {/* Описание */}
        {/* 🔥 ИСПРАВЛЕНО: Добавлен prefetch={false} */}
        <Link href={`/blog/${post.slug}`} prefetch={false} className="block flex-grow mb-6">
...
export default function BlogFeed({ initialPosts = [], categories = [] }: BlogFeedProps) {
  ...
  // Сохранили сортировку как в оригинале (на всякий случай, хотя на сервере мы уже отсортировали)
          ...
          <div className="mb-16 md:mb-24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {top3Posts.map((post, index) => (
                <PostCard key={post.id} post={post} categories={categories} priority={index === 0} />
              ))}
            </div>
          </div>
        )}
        ...
        <div className="mb-16 md:mb-24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {feedPosts.map((post, index) => (
              <PostCard key={post.id} post={post} categories={categories} priority={!isDefaultView && index === 0} />
            ))}
          </div>
