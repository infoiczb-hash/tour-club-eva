...
export default function BlogFeed({ initialPosts = [], categories = [] }: BlogFeedProps) {
 ...
 const openContactModal = useModalStore((state) => state.openContactModal);
  ...
  // ✅ УМНАЯ ЛОГИКА "ТОП-3" (Выбор редакции)
  ...
  // Топ-3 показываем только если мы на главной вкладке "Все"
  if (isDefaultView) {
      ...
      // Если выбран фильтр категории, показываем просто сетку
      feedPosts = sortedPosts;
  }

  // ✅ ПЕРЕИСПОЛЬЗУЕМЫЙ КОМПОНЕНТ КАРТОЧКИ СТАТЬИ
  ...
  return (
    <div className="min-h-screen bg-[#0B1120] text-white pb-24">
      ...
      <div className="relative pt-24 pb-8 md:pt-32 md:pb-12 border-b border-white/5 overflow-hidden">
         <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[500px] bg-teal-900/10 md:md:blur-[120px] rounded-full pointer-events-none" />
