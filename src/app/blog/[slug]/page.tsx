...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  if (absoluteImageUrl.startsWith('/')) absoluteImageUrl = `${BASE_URL}${absoluteImageUrl}`;

  // FIX 1: jsonLd сериализуется через JSON.stringify — раньше сюда уходил post.content (HTML статьи)
  const jsonLd = [
    {
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: 'Журнал', item: `${BASE_URL}/blog` },
        { '@type': 'ListItem', position: 3, name: post.title, item: postUrl }
      ]
    }
  ];

  // FIX 2: Санитизируем контент статьи здесь один раз для рендера
  ...
  return (
    <>
      ...
      <article className="min-h-screen bg-[#0B1120] pb-10 md:pb-24">
        ...
        {/* FIX 1: JSON.stringify(jsonLd) вместо post.content */}
        <script
        ...
        <div className="relative min-h-[75svh] md:min-h-[65vh] w-full overflow-hidden flex flex-col justify-end">
          ...
          <div className="absolute inset-0 z-0">
            <Image
              ...
              alt={post.title}
              fill
              // FIX 3: priority + fetchPriority="high" — LCP элемент должен грузиться немедленно.
              // Lighthouse: "Требуется значение fetchpriority=high"
              priority
              fetchPriority="high"
              // FIX 4: sizes отражает реальный контейнер (100vw = hero на весь экран, корректно)
              sizes="100vw"
              quality={85}
          ...
          <div className="relative z-10 container mx-auto px-4 pt-20 md:pt-24 pb-8 md:pb-16 max-w-7xl mt-auto">
            <Link
              href="/blog"
              className="inline-flex items-center gap-3 px-4 py-2 md:px-5 md:py-2.5 bg-white/10 hover:bg-white/20 border border-white/10 backdrop-blur-md rounded-full text-slate-200 hover:text-white transition-all mb-8 group w-fit shadow-lg"
            >
            ...
            <div className="flex flex-wrap items-center gap-4 md:gap-6 text-sm">
              <div className="flex items-center gap-3">
                <div className="relative w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden border-2 border-white/20 bg-slate-800 shadow-md shrink-0">
                  {post.author_image ? (
                    <Image
                      ...
                      fill
                      // FIX 5: avatar маленький — sizes точно описывает контейнер 48px
                      sizes="48px"
                      className="object-cover"
                    />
                  ) : (
                ...
                <div>
                  ...
                  <div className="text-slate-400 text-[11px] md:text-[12px]">
                    {post.author_role || "Гид клуба"}
                  </div>
                </div>
              </div>

              <div className="h-6 w-px bg-white/20 hidden md:block" aria-hidden="true" />
              ...
              <div className="flex items-center gap-4 text-slate-300 text-[12px] md:text-[13px] font-medium bg-slate-900/50 backdrop-blur-sm px-4 py-2 rounded-xl border border-white/5">
                <div className="flex items-center gap-1.5">
                  <Calendar size={14} className="text-teal-500" aria-hidden="true" />
                  <time dateTime={isoDate}>{formatDate(post.date)}</time>
                </div>
                <div className="w-1 h-1 rounded-full bg-slate-600" aria-hidden="true" />
                <div className="flex items-center gap-1.5">
                  <Clock size={14} className="text-teal-500" aria-hidden="true" />
        ...
        <div className="container mx-auto px-4 max-w-7xl mt-8 md:mt-16">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16">
            ...
            <div className="lg:col-span-8">
              ...
              <div className="mb-8 md:mb-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  {/* p вместо h2 — это декоративный лейбл, не структурный заголовок */}
              ...
              {/* FIX 2: safeContent — DOMPurify применён один раз выше, не сырой post.content */}
              <div
            ...
            {/* SIDEBAR */}
            <aside className="lg:col-span-4 relative mt-12 lg:mt-0" aria-label="Дополнительные материалы">
              <div className="lg:sticky lg:top-28 space-y-6 md:space-y-8 pb-10">
                ...
                <div className="p-6 md:p-8 rounded-3xl bg-slate-900/50 border border-white/5 backdrop-blur-sm shadow-xl">
                  <h2 className="text-sm md:text-base font-black uppercase tracking-widest text-white mb-6 flex items-center gap-3">
                    <BookOpen size={18} className="text-teal-500" aria-hidden="true" />
                    Читайте также
                  </h2>
                  ...
                  {relatedPosts.length > 0 ? (
                    <div className="flex flex-col gap-6">
                      {relatedPosts.map(relPost => (
                        <Link
                          ...
                          className="group flex flex-col sm:flex-row lg:flex-col xl:flex-row gap-4 items-start border-b border-white/5 pb-6 last:border-0 last:pb-0"
                        >
                          <div className="relative w-full sm:w-24 lg:w-full xl:w-24 aspect-[4/3] sm:aspect-square lg:aspect-[4/3] xl:aspect-square shrink-0 rounded-xl overflow-hidden bg-slate-800 border border-white/5 shadow-md">
                            <Image
                              ...
                              alt={relPost.title}
                              fill
                              // FIX 6: sizes точно описывает реальный контейнер сайдбара (не 100vw).
                              // lg:col-span-4 из 12 = ~33vw, внутри xl:w-24 = 96px фиксированно.
                              // Это убирает загрузку 3MB изображений вместо 96px превью.
                              sizes="(max-width: 640px) 100vw, (max-width: 1024px) 96px, (max-width: 1280px) 33vw, 96px"
                    ...
                    <div className="p-6 border border-dashed border-white/10 rounded-2xl text-center">
                      <p className="text-slate-400 text-sm font-medium">Нет других статей</p>
                    </div>
                  )}
                  ...
                  <div className="mt-8 pt-6 border-t border-white/5">
                    <Link
                      href="/blog"
                      className="flex items-center justify-center gap-2 w-full py-4 rounded-xl bg-slate-800 text-white font-bold uppercase text-[13px] hover:bg-teal-500 hover:text-slate-900 transition-all border border-white/10 hover:border-teal-500 shadow-lg group"
                    >
                ...
                <div className="p-8 rounded-3xl bg-gradient-to-br from-teal-900/40 to-slate-900 border border-teal-500/20 text-center shadow-2xl relative overflow-hidden group">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-teal-500/10 blur-2xl rounded-full pointer-events-none group-hover:bg-teal-500/20 transition-colors duration-500" aria-hidden="true" />
                  ...
                  <Link
                    href="/tour"
                    className="relative z-10 flex w-full justify-center items-center gap-2 py-4 rounded-xl bg-teal-500 text-slate-950 font-black uppercase text-[13px] tracking-wider hover:bg-teal-400 hover:scale-105 active:scale-95 transition-all shadow-[0_0_20px_rgba(20,184,166,0.3)]"
                  >
                    Выбрать маршрут <ArrowRight size={16} aria-hidden="true" />
                  </Link>
                </div>
