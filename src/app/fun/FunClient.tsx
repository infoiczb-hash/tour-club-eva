...
export default function FunClient({ activeTests }: { activeTests: FunTest[] }) {
  ...
  // ✅ Читаем URL только один раз при загрузке (отвязка от Next.js Router)
  useEffect(() => {
  ...
  // ✅ Умное открытие: меняем стейт и бесшумно переписываем URL
  const handleOpenQuiz = (slug: string) => {
    setActiveQuizSlug(slug);
    window.history.pushState(null, '', `?quiz=${slug}`);
  };

  // ✅ Умное закрытие: очищаем стейт и убираем параметр из URL
  ...
  // Находим компонент АКТИВНОЙ модалки
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 selection:bg-indigo-500/30 overflow-hidden relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
                    {tests.map((test, index) => {
                       ...
                       return (
                         <QuizCard 
                            key={test.id}
                            onClick={() => handleOpenQuiz(test.slug)} // ✅ Вызываем нашу умную функцию
                            image={test.image || ""}
                            color={visual.color}
                            icon={visual.icon}
                            badge={visual.badge}
      ...
      {/* ✅ Рендерим СТРОГО ОДНУ активную модалку, если она есть */}
      {ActiveModal && (
        <ActiveModal 
          isOpen={true} 
          open={true}   
