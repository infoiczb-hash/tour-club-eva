    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pb-28 md:pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
        ...
        <div className="space-y-6 flex flex-col">
          ...
          <div className="bg-slate-900/60 border border-white/5 rounded-3xl p-6 shadow-xl">
            ...
            <div>
              <label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 ml-1 mb-2 block">
                Диета и аллергии
              </label>
      ...
      <div className="pt-6 flex justify-end">
        <button
          type="submit"
          disabled={isPending}
          className="w-full sm:w-auto px-8 py-4 flex items-center justify-center gap-3 bg-teal-500 hover:bg-teal-400 disabled:bg-slate-800 disabled:text-slate-500 text-slate-950 font-black uppercase tracking-widest rounded-xl transition-all shadow-[0_0_20px_rgba(20,184,166,0.3)] hover:shadow-[0_0_25px_rgba(20,184,166,0.5)] active:scale-[0.98]"
        >
...
async function getDashboardData(userId: string) {
  ...
  // 3. ✅ ИСПРАВЛЕНА ЛОГИКА СТАТИСТИКИ (СИНХРОН С ИСТОРИЕЙ)
  const pastConfirmedBookings = await prisma.booking.findMany({
    where: { 
      memberId: profile.id, 
      status: { not: 'cancelled' }, // Считаем все не отмененные (как в истории)
      tourDate: { startDate: { lt: now } }
    },
    include: { 
...
export default async function DashboardPage() {
  ...
  // Берем самый ближайший тур для вывода QR-кода на карту
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-8 pb-10">
      ...
      {/* ── БЛОК 1: Главная сетка (Смарт-адаптация Mobile/Desktop) ── */}
      <div className="flex flex-col xl:flex-row gap-6 xl:gap-8 items-start">
        
        {/* ЛЕВАЯ КОЛОНКА (На мобилке - ПЕРВАЯ) */}
        ...
        {/* ПРАВАЯ КОЛОНКА (На мобилке - ВТОРАЯ и ТРЕТЬЯ) */}
        <div className="w-full xl:w-7/12 flex flex-col gap-6 order-2">
          
          {/* СТАТИСТИКА (На мобилке идет ДО баланса: order-1) */}
          ...
          {/* КОМПАКТНЫЙ БАЛАНС (На мобилке идет ПОСЛЕ статистики: order-2) */}
          <div className="order-2 xl:order-1 px-2 md:px-0">
            <div className="bg-slate-900 border border-amber-500/20 rounded-3xl overflow-hidden shadow-lg">
              ...
              {/* Раскрывающаяся часть (Нативный HTML <details>) */}
      ...
      {/* ── БЛОК 2: ДОСТИЖЕНИЯ (Горизонтальный скролл) ── */}
      <section className="pt-2">
        <AchievementsBox stats={achievements} />
      </section>

      {/* ── БЛОК 3: РЕФЕРАЛЬНАЯ ПРОГРАММА ── */}
